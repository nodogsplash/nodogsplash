...
iptables_fw_counters_update(void)
{
	...
	while (!feof(output)) {
		...
		if (3 == rc && !strcmp(target, "MARK")) {
			...
			if ((p1 = client_list_find_by_ip(ip))) {
				unsigned long long total = counter + p1->outgoing_offset;
				if (p1->counters.outgoing < total) {
					p1->counters.outgoing = total;
					p1->counters.last_updated = time(NULL);
					debug(LOG_DEBUG, "%s - Updated counter.outgoing to %llu bytes.  Updated last_updated to %d", ip, total, p1->counters.last_updated);
				}
			} else {
	...
	while (!feof(output)) {
		...
		if (3 == rc && !strcmp(target, "ACCEPT")) {
			...
			if ((p1 = client_list_find_by_ip(ip))) {
				unsigned long long total = counter + p1->incoming_offset;
				if (p1->counters.incoming < total) {
					p1->counters.incoming = total;
					debug(LOG_DEBUG, "%s - Updated counter.incoming to %llu bytes", ip, total);
				}
			} else {
				debug(LOG_WARNING, "Could not find %s in client list", ip);
			}
