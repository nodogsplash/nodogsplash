...
#define JSON_GET_FIELD_OBJECT(_target, _err, _jsn_obj, _field, _jsn_type) do { \
		...
		if (!json_object_is_type(jsn_ptr, json_type_object)) { \
...
#define JSON_GET_FIELD(_target, _err, _jsn_obj, _field, _jsn_type, _jsn_func) do { \
		...
		json_type jsn_type = _jsn_type; \
		if (!jsn_ptr) { \
			debug(LOG_ERR, "Failed to get " #_field); \
			goto err; \
		} \
		if (!json_object_is_type(jsn_ptr, jsn_type)) { \
			debug(LOG_ERR, "Wrong type for field " #_field " found type %s, expected %s", \
					json_type_to_name(json_object_get_type(jsn_ptr)), \
					json_type_to_name(_jsn_type)); \
			goto err; \
...
state_file_import_client(json_object *json_client)
{
	...
	t_client *client = client_list_add_client(mac, ip);
	if (!client) {
		debug(LOG_ERR, "Failed to add client with mac %s, ip %s. Maybe invalid mac or ip? Or duplicate entry?", mac, ip);
		return -1;
	}
	...
	JSON_GET_FIELD(client->id, err, json_client, "id", json_type_int, json_object_get_uint64);
...
state_file_import(const char *path)
{
	...
	for (int i = 0; i < len; i++) {
		...
		state_file_import_client(client);
	}
	UNLOCK_CLIENT_LIST();
